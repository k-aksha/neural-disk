...
Cedinia is a new Android touch friendly GUI frontend for NeuralDisk Core, built with Slint.

Supports the same scanning tools as NeuralDisk (duplicates, similar images and videos, empty files and folders, large files, invalid extensions, duplicate music, and more). The only missing features are similar video detection based on frames, detection of corrupted video files, and video optimization, all of which require FFmpeg.
