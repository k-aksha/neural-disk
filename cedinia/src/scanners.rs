...
use neuraldisk_core::common::traits::Search;
...
pub(crate) fn scan_duplicate_files<H: ScanResultHandler>(
    dirs: Vec<PathBuf>,
    check_method: neuraldisk_core::common::model::CheckingMethod,
    hash_type: neuraldisk_core::common::model::HashType,
    filters: &CommonFilters,
    stop: &Arc<AtomicBool>,
    handler: &Arc<H>,
    scan_id: u32,
) -> Vec<FileItem> {
    use neuraldisk_core::common::model::CheckingMethod;
    use neuraldisk_core::tools::duplicate::{DuplicateEntry, DuplicateFinder, DuplicateFinderParameters};
...
pub(crate) fn scan_similar_images<H: ScanResultHandler>(
    dirs: Vec<PathBuf>,
    similarity_preset: neuraldisk_core::tools::similar_images::SimilarityPreset,
    hash_size: u8,
    hash_alg: neuraldisk_core::re_exported::HashAlg,
    image_filter: neuraldisk_core::re_exported::FilterType,
    geometric_invariance: neuraldisk_core::tools::similar_images::GeometricInvariance,
    ...
    use neuraldisk_core::tools::similar_images::{ImagesEntry, SimilarImages, SimilarImagesParameters, return_similarity_from_similarity_preset};
...
pub(crate) fn scan_empty_files<H: ScanResultHandler>(dirs: Vec<PathBuf>, filters: &CommonFilters, stop: &Arc<AtomicBool>, handler: &Arc<H>, scan_id: u32) -> Vec<FileItem> {
    use neuraldisk_core::tools::empty_files::EmptyFiles;
    let (ptx, fwd) = spawn_progress_forwarder(Arc::clone(handler), scan_id);
...
pub(crate) fn scan_big_files<H: ScanResultHandler>(
    dirs: Vec<PathBuf>,
    search_mode: neuraldisk_core::tools::big_file::SearchMode,
    count: usize,
    filters: &CommonFilters,
    stop: &Arc<AtomicBool>,
    handler: &Arc<H>,
    ...
    use neuraldisk_core::tools::big_file::{BigFile, BigFileParameters};
    let (ptx, fwd) = spawn_progress_forwarder(Arc::clone(handler), scan_id);
...
pub(crate) fn scan_bad_extensions<H: ScanResultHandler>(dirs: Vec<PathBuf>, filters: &CommonFilters, stop: &Arc<AtomicBool>, handler: &Arc<H>, scan_id: u32) -> Vec<FileItem> {
    use neuraldisk_core::tools::bad_extensions::{BadExtensions, BadExtensionsParameters};
    let (ptx, fwd) = spawn_progress_forwarder(Arc::clone(handler), scan_id);
...
pub(crate) fn scan_bad_names<H: ScanResultHandler>(
    ...
    use neuraldisk_core::tools::bad_names::{BadNames, BadNamesParameters, NameIssues};
    let (ptx, fwd) = spawn_progress_forwarder(Arc::clone(handler), scan_id);
...
pub(crate) fn scan_exif_remover<H: ScanResultHandler>(dirs: Vec<PathBuf>, filters: &CommonFilters, stop: &Arc<AtomicBool>, handler: &Arc<H>, scan_id: u32) -> Vec<FileItem> {
    use neuraldisk_core::tools::exif_remover::{ExifRemover, ExifRemoverParameters};
    let (ptx, fwd) = spawn_progress_forwarder(Arc::clone(handler), scan_id);
...
pub(crate) fn scan_same_music<H: ScanResultHandler>(
    ...
    use neuraldisk_core::tools::same_music::{MusicSimilarity, SameMusic, SameMusicParameters};
    let (ptx, fwd) = spawn_progress_forwarder(Arc::clone(handler), scan_id);
    ...
    drop(ptx);
    fwd.join().expect("Failed to join progress forwarder thread");
    use neuraldisk_core::tools::same_music::MusicEntry;
    let use_ref = tool.get_use_reference();
