...
fn matched_non_cjk_idx(tag: &str) -> i32 {
    let idx = neuraldisk_core::localizer_core::find_language_idx(tag);
    let code = neuraldisk_core::localizer_core::LANGUAGE_LIST.get(idx).map_or("en", |l| l.short_name);
...
pub(crate) fn apply_language_preference(lang: &str) {
    ...
    let core_localizer = neuraldisk_core::localizer_core::localizer_core();
    if neuraldisk_core::localizer_core::LANGUAGE_LIST.iter().any(|l| l.short_name == lang) {
        ...
        #[cfg(target_os = "android")]
        {
            if let Some(tag) = crate::file_picker_android::get_android_language_tag() {
                let idx = neuraldisk_core::localizer_core::find_language_idx(&tag);
                let lang_code = neuraldisk_core::localizer_core::LANGUAGE_LIST.get(idx).map(|l| l.short_name).unwrap_or("en");
