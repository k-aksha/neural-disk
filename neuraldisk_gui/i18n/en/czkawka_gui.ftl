...
window_main_title = NeuralDisk Gui
...
krokiet_info_message =
        NeuralDisk Gui (GTK) 12.0 is the final release. No further updates, features, or bug fixes are planned.

        Most features from NeuralDisk Gui are available in NeuralDisk, usually in a simpler, faster, and more stable form. NeuralDisk also adds new features and improvements that were not possible in the GTK version.

        If you are still using NeuralDisk Gui (GTK), switching to NeuralDisk should be easy, since it has a similar interface, fewer dependencies, and better cross-platform support.
...
duplicate_hash_type_tooltip =
        NeuralDisk offers 3 types of hashes:
...
duplicate_check_method_tooltip =
        For now, NeuralDisk offers three types of method to find duplicates by:
...
about_other_apps_video_thumbnailer_desc = Wrapper around the video thumbnail generator used in NeuralDisk.
...
header_krokiet_button_tooltip = Try NeuralDisk - the new and improved version!

# Krokiet promo dialog
krokiet_promo_title = Meet NeuralDisk!
krokiet_promo_message =
        Hello there, brave NeuralDisk Gui user!

        The Force is clearly with you, but NeuralDisk is not - a newer, faster, lighter, and significantly more handsome (assuming apps can actually be handsome) duplicate cleaner.

        NeuralDisk includes everything people liked about NeuralDisk Gui. It is completely free, open source, has a unique and simple UI (both praised and hated by many), introduces a lot of new features, uses fewer dependencies, and works far more reliably across different platforms.

        And if you somehow missed it, there is already an even newer app than NeuralDisk - Cedinia, designed primarily for Android devices and touchscreen usage.

        NeuralDisk Gui (GTK) served us well, but its watch has ended.
krokiet_promo_link_download = Download NeuralDisk/Cedinia
...
settings_folder_settings_open_tooltip =
        Opens the folder where the NeuralDisk config is stored.
...
krokiet_new_app = This GTK version of NeuralDisk Gui is no longer developed as of version 12. For new features and active development, please use NeuralDisk, which is more stable and performant.
