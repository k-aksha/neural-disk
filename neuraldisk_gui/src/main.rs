...
use neuraldisk_core::common::basic_gui_cli::{CliResult, process_cli_args};
use neuraldisk_core::common::config_cache_path::{get_config_cache_path, print_infos_and_warnings, set_config_cache_path};
...
use neuraldisk_core::common::{get_number_of_threads, set_number_of_threads};
use glib::ExitCode;
...
fn main() {
    ...
    neuraldisk_core::common::build_runtime_info::BuildRuntimeInfo::get();
    ...
    let exists_krokiet_info_file = get_config_cache_path().is_some_and(|cache_config| {
        ...
        if !exists {
            let _ = std::fs::write(
                file_path,
                "This file indicates that user has seen NeuralDisk info dialog. You can delete this file to see the dialog again.",
            );
        }
        exists
    });
    ...
    #[cfg(target_os = "linux")]
    glib::set_prgname(Some("io.neuraldisk.neuraldisk_gui"));
    ...
    application.connect_command_line(move |app, cmdline| {
        ...
        setup_logger(false, "neuraldisk_gui", filtering_messages);
        print_version_mode("NeuralDisk gtk");
        print_infos_and_warnings(config_cache_path_set_result.infos.clone(), config_cache_path_set_result.warnings.clone());
        neuraldisk_core::common::build_runtime_info::BuildRuntimeInfo::get().log_runtime_summary();
...
fn build_ui(application: &Application, cli_args: Option<&CliResult>, needs_to_open_dialog_about_krokiet: bool) {
    ...
    // Show NeuralDisk info dialog if needed
...
pub(crate) fn print_czkawka_gui_info(thread_number: usize) {
    ...
    info!("NeuralDisk Gui - used thread number: {thread_number}, gtk version {gtk_version}");
}
