# neuraldisk_gui – Architecture Guide

## Status: Maintenance mode only

No new features. Bug-fixes that keep it compatible with `neuraldisk_core` API
...
available in `neuraldisk_core` are exposed via a notebook (tab-based) interface.
...
│   ├── connect_krokiet_info_dialog.rs  # One-time NeuralDisk deprecation notice (shown on first run)
│   ├── connect_krokiet_promo_dialog.rs # NeuralDisk promo dialog (shown from the header button)
...
2. Call `localizer.select([lang_id])` for both `neuraldisk_core` and `neuraldisk_gui`.
...
JSON file at `~/.config/neuraldiskcli/czkawka_gui_config.json` (Linux; shares its config folder
with `neuraldisk_cli`, see `set_config_cache_path` call sites). Struct `SettingsJson`
