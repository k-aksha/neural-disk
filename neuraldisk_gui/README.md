![neuraldisk_gui_logo](https://user-images.githubusercontent.com/41945903/102616149-66490400-4137-11eb-9cd6-813b2b070834.png)

NeuralDisk Gui is a graphical user interface for NeuralDisk Core, built with GTK 4.
...
**Version 12.0 is the last released version of NeuralDisk Gui (GTK).** No new binaries will be provided. All users are encouraged to migrate to [NeuralDisk](../neuraldisk/README.md), the new Slint-based GUI.

The source code remains in the repository and compatibility with `neuraldisk_core` will be maintained for some time, so the application can still be built manually. However, all issues and feature requests specific to NeuralDisk Gui (GTK) will be closed, except for critical problems affecting widely used unofficial builds such as Debian or Docker packages.

## Requirements

Requirements depend on your platform.

NeuralDisk Gui (GTK) is in maintenance mode and no new prebuilt binaries are provided for it in
this fork - see the deprecation notice above. The last prebuilt binaries (as Czkawka Gui, before
the rebrand) are available from the upstream project's releases page:
...
After installing the required dependencies, download the last prebuilt binaries (as Czkawka Gui) from the upstream project's [releases page](https://github.com/qarmin/czkawka/releases).
...
```
Note: this installs the upstream Czkawka Gui flatpak, not this fork - there is no separate
Flatpak listing for NeuralDisk Gui.
...
The file should be installed to `C:\msys64\mingw64\bin\czkawka_gui.exe` (unofficial MSYS2 package, not yet renamed upstream) and can be run from there.  
This version is likely the most feature-complete on Windows, as it is compiled with optional features enabled.

## Compilation

Compiling the GUI is more complex than compiling the CLI, core, or NeuralDisk, because it uses GTK4 (written in C) and requires many build and runtime dependencies.
...
cargo run --release --bin neuraldisk_gui
# Or with support for heif, libraw, libavif
cargo run --release --bin neuraldisk_gui --features "heif,libraw,libavif"
```
...
This app was originally named Czkawka (Polish for "hiccup") under the upstream project of the
same name, and is rebranded here as NeuralDisk Gui. See the root [README](../README.md) for
