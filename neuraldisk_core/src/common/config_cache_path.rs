...
/// calling `set_config_cache_path`, so that `neuraldisk_core` stays package-agnostic.
///
/// Android paths used:
...
pub fn set_config_cache_path(cache_name: &'static str, config_name: &'static str) -> ConfigCachePathSetResult {
    ...
    // LinFlatpak: /home/username/.var/app/io.neuraldisk.neuraldisk_gui/config/czkawka
    // Win: C:\Users\Username\AppData\Roaming\neuraldisk\NeuralDisk\config
    // Mac: /Users/Username/Library/Application Support/io.neuraldisk.NeuralDisk
    ...
    let default_cache_folder = ProjectDirs::from("io", "neuraldisk", cache_name)
        .map(|proj_dirs| proj_dirs.cache_dir().to_path_buf())
        .or(android_cache_folder);
    let default_config_folder = ProjectDirs::from("io", "neuraldisk", config_name)
