...
fn clean_exif_single_file(file_path: &str, tag_groups: &[String], tags_u16: &[String], override_file: bool) -> Result<(), String> {
    // Such data are split into multiple vectors, but in NeuralDisk they are not changed
    ...
    let _ = neuraldisk_core::tools::exif_remover::core::clean_exif_tags(file_path, &connected_tags, override_file)
