...
models. All scanning logic lives in `neuraldisk_core`.
...
| File | Content |
|------|---------|
| `~/.config/neuraldisk/config_general.json` | `BasicSettings` – default preset index, preset names, theme, window size |
| `~/.config/neuraldisk/config_preset_N.json` | `SettingsCustom` – paths, extensions, tool parameters |
...
- `heif`, `libraw`, `libavif`, `xdg_portal_trash` – forwarded to `neuraldisk_core`
