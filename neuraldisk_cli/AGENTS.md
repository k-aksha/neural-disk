# neuraldisk_cli – Architecture Guide

## Overview

`neuraldisk_cli` is a thin CLI wrapper around `neuraldisk_core`. It handles argument
parsing, thread orchestration, progress rendering, and result output. All
scanning logic lives in `neuraldisk_core`; this crate only wires it to the
terminal.
...
and the calculation thread. The tools in `neuraldisk_core` poll it during scanning
...
| `neuraldisk_core`     | All scanning logic               |
