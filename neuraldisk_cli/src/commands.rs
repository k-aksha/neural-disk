...
use neuraldisk_core::CZKAWKA_VERSION;
use neuraldisk_core::common::model::{CheckingMethod, HashType};
use neuraldisk_core::common::tool_data::DeleteMethod;
use neuraldisk_core::re_exported::{FilterType, HashAlg};
use neuraldisk_core::tools::broken_files::CheckedTypes;
use neuraldisk_core::tools::same_music::MusicSimilarity;
use neuraldisk_core::tools::similar_images::GeometricInvariance;
use neuraldisk_core::tools::similar_videos::{
...
use neuraldisk_core::tools::video_optimizer::{NoiseReductionMethod, VideoCodec};
use log::error;
...
pub enum Commands {
    #[clap(
        ...
        after_help = "EXAMPLE:\n    neuraldisk_cli dup -d /home/rafal -e /home/rafal/Obrazy  -m 25 -x 7z rar IMAGE -s hash -f results.txt -D aeo"
    )]
    Duplicates(DuplicatesArgs),
    #[clap(
        name = "empty-folders",
        about = "Finds empty folders",
        after_help = "EXAMPLE:\n    neuraldisk_cli empty-folders -d /home/rafal/rr /home/gateway -f results.txt"
    )]
    EmptyFolders(EmptyFoldersArgs),
    #[clap(
        name = "big",
        about = "Finds big files",
        after_help = "EXAMPLE:\n    neuraldisk_cli big -d /home/rafal/ /home/piszczal -e /home/rafal/Roman -n 25 -J -x VIDEO -f results.txt"
    )]
    BiggestFiles(BiggestFilesArgs),
    #[clap(
        name = "empty-files",
        about = "Finds empty files",
        after_help = "EXAMPLE:\n    neuraldisk_cli empty-files -d /home/rafal /home/szczekacz -e /home/rafal/Pulpit -R -f results.txt"
    )]
    EmptyFiles(EmptyFilesArgs),
    #[clap(
        name = "temp",
        about = "Finds temporary files",
        after_help = "EXAMPLE:\n    neuraldisk_cli temp -d /home/rafal/ -E */.git */tmp* *Pulpit -f results.txt -D"
    )]
    Temporary(TemporaryArgs),
    #[clap(
        name = "image",
        about = "Finds similar images",
        after_help = "EXAMPLE:\n    neuraldisk_cli image -d /home/rafal/ -E */.git */tmp* *Pulpit -f results.txt"
    )]
    SimilarImages(SimilarImagesArgs),
    #[clap(name = "music", about = "Finds same music by tags", after_help = "EXAMPLE:\n    neuraldisk_cli music -d /home/rafal -f results.txt")]
    SameMusic(SameMusicArgs),
    #[clap(
        name = "symlinks",
        about = "Finds invalid symlinks",
        after_help = "EXAMPLE:\n    neuraldisk_cli symlinks -d /home/kicikici/ /home/szczek -e /home/kicikici/jestempsem -x jpg -f results.txt"
    )]
    InvalidSymlinks(InvalidSymlinksArgs),
    #[clap(
        name = "broken",
        about = "Finds broken files",
        after_help = "EXAMPLE:\n    neuraldisk_cli broken -d /home/kicikici/ /home/szczek -e /home/kicikici/jestempsem -x jpg -f results.txt"
    )]
    BrokenFiles(BrokenFilesArgs),
    #[clap(name = "video", about = "Finds similar video files", after_help = "EXAMPLE:\n    neuraldisk_cli video -d /home/rafal -f results.txt")]
    SimilarVideos(SimilarVideosArgs),
    #[clap(
        name = "ext",
        about = "Finds files with invalid extensions",
        after_help = "EXAMPLE:\n    neuraldisk_cli ext -d /home/czokolada/ -f results.txt"
    )]
    BadExtensions(BadExtensionsArgs),
    #[clap(
        name = "bad-names",
        about = "Finds files with bad names",
        after_help = "EXAMPLE:\n    neuraldisk_cli bad-names -d /home/rafal -f results.txt"
    )]
    BadNames(BadNamesArgs),
    #[clap(
        name = "video-optimizer",
        about = "Optimizes video files (transcode or crop)",
        after_help = "EXAMPLE:\n    neuraldisk_cli video-optimizer -d /home/rafal -f results.txt"
    )]
    VideoOptimizer(VideoOptimizerArgs),
    #[clap(
        name = "exif-remover",
        about = "Finds and removes EXIF tags from images",
        after_help = "EXAMPLE:\n    neuraldisk_cli exif-remover -d /home/rafal -f results.txt"
    )]
...
pub struct SimilarVideosArgs {
    ...
    #[clap(
        ...
        long_help = "Generate thumbnails for found similar videos. Thumbnail preview is only available in GUI (NeuralDisk/Cedinia), but generating them via CLI can pre-populate the cache for later GUI usage."
    )]
...
pub struct ExifRemoverArgs {
    ...
    #[clap(
        ...
        long_help = "Override original files instead of creating a copy with 'neuraldisk_cleaned_exif' inserted before the extension (e.g. 'photo.jpg' becomes 'photo.neuraldisk_cleaned_exif.jpg')"
    )]
