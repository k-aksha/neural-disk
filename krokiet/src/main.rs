...
use neuraldisk_core::common::basic_gui_cli::process_cli_args;
use neuraldisk_core::common::config_cache_path::{print_infos_and_warnings, set_config_cache_path};
use neuraldisk_core::common::ffmpeg_utils::{check_if_ffprobe_ffmpeg_exists, get_working_hardware_encoders};
...
use neuraldisk_core::tools::video_optimizer::HardwareEncoder;
...
use crate::connect_agent_chat::connect_agent_chat;
...
fn main() {
    ...
    let config_cache_path_set_result = set_config_cache_path("NeuralDisk", "NeuralDisk");
    let cli_args = process_cli_args("NeuralDisk", "neuraldisk_gui", std::env::args().skip(1).collect());
    ...
    setup_logger(false, "neuraldisk", filtering_messages);
    print_version_mode("NeuralDisk");
...
pub(crate) fn print_available_hw_accelerations() {
    let hw_accels = neuraldisk_core::common::ffmpeg_utils::get_available_hw_accelerations();
...
pub(crate) fn print_krokiet_features() {
    ...
    info!("NeuralDisk features({}): [{}]", features.len(), features.join(", "));
}
